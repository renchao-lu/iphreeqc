 ...
 */
    ...
    *---------------------------------------------------------------------- */
    ...
    count_elements = 0;
    for (auto& element : pSrc->elements)
    {
        auto ele_name = element.name;
        auto compare_by_name = [&ele_name](auto const& item) {
            return item.name == ele_name;
        };

        string_hsave(element.name);
        auto elt_ptr = BaseLib::findElementOrError(
            pSrc->elements.begin(), pSrc->elements.end(), compare_by_name,
            "Could not find kinetic reactant '" + ele_name + "'.");
        elt_ptr.gfw = element.gfw;
    }
...
//			free_check_null(name);
        ...
        //		{
        //            *master_isotope_ptr->elt =
        //            element_store(pSrc->master_isotope[i].elt->name);
        //		}
